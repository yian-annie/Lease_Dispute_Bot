    ...
    async def on_message(self, message):
        ...
        if self.user.mentioned_in(message):
            ...
            if msgSTR == 'ping':
                ...
                if mscDICT[message.author]["confirm425_BOOL"] == None and mscDICT[message.author]["confirm429_BOOL"] == None and mscDICT[message.author]["confirm_Security_Deposit_BOOL"] == None and mscDICT[message.author]["confirm_fees_BOOL"] == None and mscDICT[message.author]["confirm_comein_BOOL"] == None:
                    ...
                    if "confirm425_BOOL" in lokiResultDICT:
                        mscDICT[message.author]["confirm425_BOOL"] = lokiResultDICT["confirm425_BOOL"]
                    elif "confirm429_BOOL" in lokiResultDICT:
                        mscDICT[message.author]["confirm429_BOOL"] = lokiResultDICT["confirm429_BOOL"] 
                    elif "confirm_Security_Deposit_BOOL" in lokiResultDICT:
                        mscDICT[message.author]["confirm_Security_Deposit_BOOL"] = lokiResultDICT["confirm_Security_Deposit_BOOL"]
                    elif "confirm_fees_BOOL" in lokiResultDICT:
                        mscDICT[message.author]["confirm_fees_BOOL"] = lokiResultDICT["confirm_fees_BOOL"] 
                    elif "confirm_comein_BOOL" in lokiResultDICT:
                        mscDICT[message.author]["confirm_comein_BOOL"] = lokiResultDICT["confirm_comein_BOOL"]                    
                    
                    #不需要處理多輪對話的租賃問題
                    if mscDICT[message.author]["confirm_Security_Deposit_BOOL"] == True:
                        ...
                        replySTR = """聽起來您的問題是關於水電費、網路費、瓦斯費等方面的爭議，根據內政部頒布的「住宅租賃契約應約定及不得約定事項」，
                                    原則上水費、電費、瓦斯費、網路費、管理費等費用，基於契約自由原則，都可以由雙方約定某一方負擔，所以請您注意您的租賃契約                    書面上關於這些費用的約定。
                    ...
                    #開始處理需要多輪對話的租賃問題
                    #(一)425買賣不破租賃的問題
                    if mscDICT[message.author]["confirm425_BOOL"] == True: #確認問題為425
